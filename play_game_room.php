<?php
/**
 * play_game_room.php — Seed-pool rotation music player for the game room speaker.
 *
 * POST /HA_Jarvis/play_game_room.php
 * Optional body: {"preference": "something upbeat"}   (logged, not yet used for seed selection)
 *
...
$context  = 'game_room';
$speaker  = 'Game room speaker';
$fallback = 'upbeat pop game room playlist';
...
function _claude_fallback(string $fallback): string {
    $refined = _ask_claude_refine($fallback, null, 'game_room');
...
function _do_play(
    ...
    try {
        ...
        error_log("[play_game_room] history/seed update failed: " . $e->getMessage());
    }
}
